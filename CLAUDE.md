...
### Next
V1 is feature-complete per the brief. Nothing is queued — possible follow-ups (not started, no priority implied): tune `targetTimeMs` thresholds from real play data, flesh out the matchstick category, or anything in brief §11 (explicitly out of scope for V1: AI-generated puzzles, free-text reasoning capture, multi-child support, backend/auth/sync).
