...
- **Phase 6 complete**: **Four 4s** (`four-fours.engine.ts` + component) — all six games are now built. Structurally different from the grid games: there's no fixed solution to mask, so `generate()` brute-forces the set of integer values reachable from four 4s under the tier's allowed operators (a tiny search — 4 values, 16 subsets, every split = every parenthesization) and picks a target from whatever lands in the tier's range. Tier 1 had to use `{+, ÷}` rather than a more "introductory-feeling" pair — `{+,-}` on four equal digits can only reach multiples of 4 (every term is ±4), and `{+,×}` can't go below 16 at all (both ops only grow positive values), so division is mathematically required to hit small targets. The UI is tap-to-build (digit tiles consumed by index, reusable operator/bracket tiles), not free-text — invalid taps are disabled at construction time (alternating value/operator grammar) rather than parsed-and-rejected after the fact, with a live `= N` preview. `validate()` doesn't know about tiers/allowed-ops at all — it only checks the digit multiset and the evaluated result. Verified directly: candidate-target coverage per tier, the expression evaluator (precedence/brackets/div-by-zero/malformed input), and validate() accepting a correct cross-operator solution while rejecting wrong digit counts and wrong totals.
- **Phase 7 complete**: **Puzzle Vault** (`puzzle-vault/` — `puzzle-vault-data.ts`, `PuzzleVaultStoreService`, `vault-home`/`category-list`/`puzzle-detail` components) — **V1 is now feature-complete per the brief**. 17 hand-written puzzles (5 measuring, 5 crossing, 5 weighing, 2 matchstick), each hand-verified for a correct solution before writing (e.g. the missionaries-and-cannibals 11-trip sequence was traced step-by-step checking the safety constraint at every intermediate state, not just start/end). Deliberately scoped down from the brief's literal "move one matchstick to fix an equation" framing to 2 constructive/geometric matchstick puzzles instead — composing a correct "move-one-stick" equation fix requires stick-budget accounting that's easy to get subtly wrong without a visual aid to check against, and shipping an unverifiable wrong solution to a kid felt like the wrong tradeoff. `PuzzleVaultStoreService` is intentionally separate from `ProgressStoreService` — the Vault isn't adaptively tiered (brief §7 is explicit about this), so the rolling-attempt-window/tier-promotion machinery doesn't apply; it just tracks solved IDs per category. "Solved" is marked when the solution is revealed, not via a separate self-report step. Verified the full browse → hint → reveal-solution → marked-as-solved flow in a real browser.
...
- The "structural validation" pattern (validate against the relation/constraints, not a stored solution) ended up underpinning every game except Skip Counting and Four 4s' digit-multiset check: KenKen (multiple valid fills per cage layout), Number Pyramid (forward/reverse sharing one validator), Magic Square (any valid completion, not just the generated square).

### Remaining gaps vs. the brief (not blocking, but worth knowing about)
- Matchstick category has only 2 entries (vs. 5 for the others) — see Phase 7 note above. Good candidate to revisit with a verified visual reference.
- All `targetTimeMs` tier values across all six games are still untuned placeholder guesses — the adaptive difficulty system is implemented and working, but its thresholds haven't been calibrated against actual play data from Neo yet.

- **Persistent sidebar shell added** (post-V1): `src/app/layout/sidebar/sidebar.component.ts` — a new `layout/` folder, parallel to `shared/`/`core/`/`games/`/`home/`/`puzzle-vault/`, for app-shell chrome rather than per-game reusable primitives. `app.html` now wraps `<router-outlet>` in `<div class="flex h-screen overflow-hidden"><app-sidebar /><main class="flex-1 overflow-y-auto">...</main></div>` — the sidebar persists across every route (home, all 6 games, all 3 puzzle-vault routes) with zero changes to `app.routes.ts` or any feature component. Math is the only live subject (6 game links + `routerLinkActive` highlighting); Languages/Science are plain greyed `<span>` placeholders (no `routerLink`, no routes, no folders) — this is purely visual groundwork for the user's stated future intent to add those as sibling subjects. Puzzle Vault sits in its own section below a divider, matching the brief's framing of it as separate from the drill games; its link intentionally does NOT use `exact: true` matching so it stays highlighted while browsing into `/puzzle-vault/:category` and `/puzzle-vault/:category/:id`. `home.component.ts` got a light refresh (dropped now-dead "coming soon" branch, reframed heading) since the sidebar — not the home page — now owns primary navigation.
- No folder/route restructuring was done for the future Languages/Science subjects (deliberately deferred until those subjects actually get built — see brief §11-style reasoning: avoid speculative reorganization). No backend groundwork was started either — both are future context only, not active work.

### Next
V1 is feature-complete per the brief, with a persistent sidebar shell on top. Nothing is queued — possible follow-ups (not started, no priority implied): tune `targetTimeMs` thresholds from real play data, flesh out the matchstick category, build out Languages/Science as real subjects (would warrant revisiting the flat `games/` folder structure at that point), add a backend, or anything in brief §11 (explicitly out of scope for V1: AI-generated puzzles, free-text reasoning capture, multi-child support, backend/auth/sync).
