...
- Kakooma's tier `targetTimeMs` values (10s/15s/20s) are placeholder guesses — tune once there's real play data from Neo. Same for Skip Counting (8s/12s/20s), KenKen (25s/50s/90s), and Number Pyramid (15s/25s/45s).
...
- Dynamic Tailwind classes built via template-string interpolation (e.g. `` `border-${side}-2` ``) don't get picked up by Tailwind's static scanner — they silently produce no CSS. `kenken.component.ts`'s `sideBorder()` works around this with a literal lookup table. Number Pyramid avoided the issue entirely by sticking to static class strings; keep it in mind for Magic Square's grid rendering.
